...
public class OpenAiChatAdapter implements AiChatPort, FoodPreferenceAiPort {
    ...
    /**
     * 음식 취향 추출 프롬프트. %s에 극성별 추출 대상 규칙이 들어간다.
     *
     * <p>극성을 지정하지 않으면 "면요리는 싫고 국물은 좋아요" 같은 문장에서 두 키워드가 모두
     * 나와, 호출한 입력칸 기준으로 통째로 한쪽에 저장된다.</p>
     */
    private static final String FOOD_SYSTEM_PROMPT_FORMAT = """
            너는 음식 취향 분석기다. 사용자의 자유텍스트에서 구체적인 음식·메뉴 키워드를 뽑는다.
            반드시 아래 JSON 형식으로만 응답한다.
            {
              "foods": ["메뉴 이름", ...]
            }
    ...
    private static final String LIKE_RULE = """
            - 사용자가 좋아한다고 말한 음식만 뽑는다. 싫다·못 먹는다고 말한 음식은 절대 담지 않는다.
              (예: "면요리는 싫고 국물은 좋아요" → ["국물"]. "면요리"는 담지 않는다.)""";

    private static final String EXCLUDE_RULE = """
            - 사용자가 싫다·못 먹는다·별로라고 말한 음식만 뽑는다. 좋아한다고 말한 음식은 절대 담지 않는다.
              (예: "면요리는 싫고 국물은 좋아요" → ["면요리"]. "국물"은 담지 않는다.)""";
    ...
    public ExtractedAllergen extractAllergens(String freeText, List<String> standardAllergenNames) {
        ...
        log.info("알레르기 추출 입력:{}, 추출값: {}", freeText, content);
        return parse(content, AllergenExtractionJson.class).toDomain();
    }

    @Override
    public ExtractedFoodPreference extractFoodPreference(
            String freeText,
            List<String> menuNames,
            FoodPreferencePolarity polarity
    ) {
        String systemPrompt = FOOD_SYSTEM_PROMPT_FORMAT.formatted(
                polarity == FoodPreferencePolarity.LIKE ? LIKE_RULE : EXCLUDE_RULE);
        String userPrompt = buildUserPrompt("메뉴 목록", menuNames, freeText);
        String content = openAiJsonClient.requestJson(systemPrompt, userPrompt, EXTRACTION_MAX_TOKENS);
        log.info("선호 음식 추출 극성: {}, 입력: {}, 추출값: {}", polarity, freeText, content);
