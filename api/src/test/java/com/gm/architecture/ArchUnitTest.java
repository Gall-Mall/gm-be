 ...
 *
 * <p>api 모듈은 core/storage/client 를 모두 조립하므로, 테스트 클래스패스에서
...
class ArchUnitTest {
    ...
    /**
     * core 는 순수 도메인 모듈이다.
     * 다른 어떤 애플리케이션 모듈(api/storage/client)에도 의존해서는 안 된다.
     */
    @ArchTest
    static final ArchRule core_는_다른_모듈에_의존하지_않는다 =
     ...
     * 컴포넌트 스캔·빈 구성을 위해 storage 구성을 참조하는 것을 허용한다.
     * 컨트롤러·서비스 등 나머지 코드는 storage 를 직접 참조할 수 없다.</p>
     */
    @ArchTest
    static final ArchRule api_는_storage를_직접_호출할_수_없다 =
            noClasses().that().resideInAPackage("com.gm.api..")
                    ...
                    .resideInAnyPackage(
                            "com.gm.db..",
                            "com.gm.redis..")
                    .as("api 모듈은 storage(db/redis)를 직접 호출할 수 없다 (조립 지점 제외)");

    /**
     * storage:db 는 말단 어댑터다.
     * api·client 나 다른 storage(redis) 에 의존해서는 안 된다. (db → db 는 허용)
     */
    @ArchTest
    static final ArchRule db는_다른_모듈에_의존하지_않는다 =
            noClasses().that().resideInAPackage("com.gm.db..")
                    ...
                    .resideInAnyPackage(
                            ...
                            "com.gm.client..",
                            "com.gm.redis..")
                    .as("storage:db 모듈은 api/client/redis 에 의존할 수 없다");

    /**
     * storage:redis 는 말단 어댑터다.
     * api·client 나 다른 storage(db) 에 의존해서는 안 된다. (redis → redis 는 허용)
     */
    @ArchTest
    static final ArchRule redis는_다른_모듈에_의존하지_않는다 =
            noClasses().that().resideInAPackage("com.gm.redis..")
                    ...
                    .as("storage:redis 모듈은 api/client/db 에 의존할 수 없다");

    /**
     * client 는 외부 연동 말단 어댑터다.
     * api 나 storage(db/redis) 에 의존해서는 안 된다.
     */
    @ArchTest
    static final ArchRule client_는_다른_모듈에_의존하지_않는다 =
            noClasses().that().resideInAPackage("com.gm.client..")
                    ...
                    .allowEmptyShould(true)
                    .as("client 모듈은 api/storage 에 의존할 수 없다");

    /**
     * 모듈(최상위 com.gm.* 슬라이스) 사이에 순환 의존이 없어야 한다.
     */
    @ArchTest
    static final ArchRule 모듈간_순환_의존이_없다 =
            slices().matching("com.gm.(*)..").should().beFreeOfCycles()
                    .as("com.gm 하위 모듈 사이에 순환 의존이 없어야 한다");
}
